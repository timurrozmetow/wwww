...
EXIT=0). По выбору владельца: макет «Классический VPN», онбординг убран, язык авто.

- **Вход без гейтов:** Splash (анимация) → сразу Home. Язык **авто-определяется**
  по локали ([locale.ts](../apps/mobile/src/lib/locale.ts): TM/`tk`→tk, иначе по
  языку устройства, фолбэк ru), меняется в Настройках. Экраны выбора языка и
  онбординга (4 слайда) **удалены**.
- **Home (классический VPN):** время-баланс + ⚙ сверху, баннер, карточка сервера
  (флаг/имя/город·ping/качество-точка, tap→выбор), большая круглая кнопка Connect
  (выключена до Этапа B), «Смотреть рекламу +30» + подсказка «1 ролик = N мин».
  [ConnectButton](../apps/mobile/src/components/ConnectButton.tsx).
- **Экран выбора сервера** — «Авто» + список из `/api/vpn/servers` (флаг/ping/
  качество, выбор сохраняется). Пинг считает бэкенд, не телефон (§6).
- **Экран Настройки** — язык (ru/tr/tk), эконом-режим, приватность, версия.
- Стор: `onboarded` убран, добавлен `selectedServerId` (персист). i18n ru/tr/tk
  переписаны под новые экраны.

**Напоминание про ключи:** `happ://crypt4/…` добавляются в **админке** (VPN → Add
server → config), не в приложении — пользователь сырой конфиг не видит (§7.2).

**Проверки:** `typecheck` ✅ (mobile) · `lint`/`format` ✅ · `expo export:embed` ✅ EXIT=0.
